    ...
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                _tableId = $.common.isEmpty(options.tableId) ? "bootstrap-table" : options.tableId;
                _clickToSelect = $.common.isEmpty(options.clickToSelect) ? true : options.clickToSelect;
                ...
                $('#'+_tableId).bootstrapTable({
                    ...
                    clickToSelect: _clickToSelect,                      // 点击列选择
                });
            },
    	...
    	modal: {
            ...
            openMax: function (title, url, width, height) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
            ...
            open2: function (title, url, width, height,deleteUrl) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
            ...
            open1: function (title, url, width, height) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            // 添加信息
            add: function(id) {
                console.log("id-2020:" + id);
            	var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
            	console.log("url-2020:" + url);
                $.modal.open("添加" + $.table._option.modalName, url);
            },
            add1: function(id) {
                var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
                $.modal.open("新增" + $.table._option.modalName, url,1400);
            },

            add2: function(id,deleteUrl) {
                var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
                $.modal.open2("新增" + $.table._option.modalName, url,1400,null,deleteUrl);
            },
            ...
            edit1: function(id) {
                ...
                $.modal.open("修改" + $.table._option.modalName, url,1300,700);
            },
            edit2: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                ...
                $.modal.open1("修改" + $.table._option.modalName, url,1300,700);
            },
            // 工具栏表格树修改
            editTree: function() {
